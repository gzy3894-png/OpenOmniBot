...
| push mine · GHA · stage APK | **完成** | `1220a4d` + fix `898dc26`；GHA #29594207043；APK staged |
| 真机 §7 | 未开始 | 待设备复测 |
...
- [x] 本 EXEC / PLAN / `DELIVERY.md`（文档对齐 READY）
- [x] push mine · GHA · stage APK · 回填 commit/run/sha

## 验收

见 PLAN §7 测点 1–9；PASS 标准含：**无** 上述三类 `thread not found`；soft 写不杀 session；B34/B35 回归不回退。

真机复测优先清单：permission mode · model switch · model labels · Fast/auto-compact toggles（含 goal clear / resume 后）。

## 交付字段

| 项 | 值 |
|----|-----|
| 功能 commit | `1220a4d`（B37 stale-thread + soft conf + model display + auto-compact） |
| 编译修 commit | `898dc26`（base abstract for auto-compact compile fix） |
| HEAD | `898dc26` |
| 首包 GHA | #29593369757 **FAILED**（cross-mixin private compile）→ 由 `898dc26` 修 |
| GHA run | #29594207043 · Baseline Standard Debug · **SUCCESS** |
