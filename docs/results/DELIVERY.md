...
3. 按 **§3 测点（B1–B13）** 点测；回：`PASS` / `FAIL` + 卡在哪
...
| 副本 | `/storage/emulated/0/Download/OpenOmniBot-s1-ee07854-standard-debug.apk` |
| 状态 | **READY** |
...
| 功能 commit | `4c8ea35`（含 12bugs+B13 + plan card import hotfix） |
| CI/ship | `ee07854` · D8 heap 8g + max-workers=2 |
...
| 真源 | `PLAN-2026-07-15-user-12bugs-codex-handoff.md` + `PLAN-2026-07-15-12bugs-b13-exec.md` + `codex-cli-core-implementation-logic.zh-CN.md` |

**本包相对上一 READY（`d739f18`）修复：**

| ID | 修复 |
|----|------|
| **B13** | 空会话 `startThread` ensure 后再 `setThreadGoal`（目标模式无需先闲聊） |
...
| **B7** | **计划模式** 主区方案卡 + 批准/拒绝 |
| **B12** | compact 成败 tip + `thread/compacted` |
...
- B3 为 text 注入 `skill_path:`（bridge 尚未 structured `UserInput.skill`）
